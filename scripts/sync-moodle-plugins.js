...
function packagePlugin(plugin) {
  ...
  // Si es headlessui, asegurar compilación de Vite
  if (plugin.dirName === 'headlessui') {
  ...
  const pluginsBaseDir = path.join(projectRoot, 'plugin');
  // Moodle exige que el zip contenga la carpeta raíz del plugin (ej. headless/ o headlessui/)
...
// -------------------------------------------------------------
// 5. Instalación Web mediante Puppeteer
// -------------------------------------------------------------
async function installPluginViaWeb(page, plugin, zipPath) {
  ...
  // Esperar a que el Filepicker termine de cargar su JS/YUI y el botón sea visible
...
(async () => {
  ...
  for (const p of localPlugins) {
    console.log(`  • ${p.component.padEnd(20)} -> Versión: ${p.version} | Release: ${p.release} (${p.dirName})`);
  }
