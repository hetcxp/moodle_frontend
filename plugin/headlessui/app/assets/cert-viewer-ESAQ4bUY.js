import{n as e}from"./index-zGsh5QDx.js";import{CertService as t}from"./cert-DUucu54F.js";function n({mod:n,certData:r,issuances:i,courseId:a}){let o=Array.isArray(i)&&i.length>0,s=document.createElement(`div`);s.className=`cert-viewer`,s.style.cssText=`
  ...
  `,l.textContent=`🎓`;let u=document.createElement(`div`),d=document.createElement(`h3`);d.textContent=n.name,d.style.cssText=`margin:0; font-size:1.2rem; font-weight:600; color: var(--color-text, #fff);`;let f=document.createElement(`p`);if(f.textContent=`Certificado de finalización`,f.style.cssText=`margin:0.25rem 0 0; font-size:0.85rem; color: var(--color-text-muted, #888);`,u.appendChild(d),u.appendChild(f),c.appendChild(l),c.appendChild(u),s.appendChild(c),r?.intro){let t=document.createElement(`div`);t.className=`cert-intro`,t.style.cssText=`
    ...
    font-weight: 600;
    width: fit-content;
    ${o?`background: rgba(34,197,94,0.15); color: #4ade80; border: 1px solid rgba(34,197,94,0.3);`:`background: rgba(148,163,184,0.1); color: #94a3b8; border: 1px solid rgba(148,163,184,0.2);`}
  `,p.innerHTML=o?`<span>✅</span><span>Certificado emitido</span>`:`<span>⏳</span><span>Aún no emitido</span>`,s.appendChild(p),!o){let e=document.createElement(`div`);e.style.cssText=`
    ...
    ${o?`background: var(--color-primary, #6366f1); color: #fff; opacity: 1;`:`background: var(--color-border, #333); color: var(--color-text-muted, #666); opacity: 0.6;`}
